...
export class Arrays {
  ...
  static isArray<T>(x: Array<T> | any): x is Array<T> {
    return (
      Array.isArray && Array.isArray(x) && x && typeof x.length === 'number'
    );
  }

  static mapByField<T1, K extends keyof T1>(
    x: Array<T1>,
    key: K
  ): Array<T1[K]> {
...
export class Flow {
  static executeNTimes(
    iterations: number,
    callbackfn: (iteration: number) => void
  ): void {
    if (!callbackfn) {
      return;
    }
...
export class Numbers {
  private static preFilterArray(
    ...nums: Array<number> | Array<Array<number>>
  ): Array<number> {
    ...
    const filteredArray: Array<number> = (
      (Arrays.isArray(nums[0]) ? nums[0] : nums) as Array<number>
...
/* export class Files {
  ...
  static createAndDownloadBlobFile(
    body: BlobPart,
  ...
  static saveBlobAsFile(
    data: HttpResponse<any> | BlobPart,
    dataType: HttpEventType.Response | string,
    filename: string
  ): void {
    let blob: Blob;
...
export class Dates {
  ...
  static fechaVacia(fecha: Fecha): boolean {
    ...
    // tslint:disable-next-line:max-line-length
    if (
      Generic.isNullOrUndefined(fecha.día) &&
      Generic.isNullOrUndefined(fecha.mes) &&
      Generic.isNullOrUndefined(fecha.año)
    ) {
      return true;
    }
